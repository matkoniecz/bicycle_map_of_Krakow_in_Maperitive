...
low_traffic_road_accessible_to_bicycles = "(#{is_one_of("highway", road_low_traffic_list)} AND NOT bicycle=no AND NOT #{no_access})"
...
cycleway = "((highway=cycleway OR bicycle=designated) AND (segregated = yes OR NOT (foot=yes OR foot=designated)) AND NOT #{no_access})"
...
unexpected_allowed_cycling = "(bicycle=yes AND NOT #{no_access} AND NOT area=yes AND (highway=footway OR highway=path OR highway=pedestrian))"
...
separate_carriageway_rather_than_true_oneway = "(highway=motorway OR highway=primary OR highway=secondary OR #{link} OR dual_carriageway=yes OR dual_carriageway:bicycle=yes OR bicycle=use_sidepath)"

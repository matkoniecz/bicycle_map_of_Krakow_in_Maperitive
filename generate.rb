...
__contraflow = "((cycleway=opposite_lane OR oneway:bicycle = no) AND NOT #{__no_access})"
...
__unexpected_cycling_ban = "((bicycle=no AND #{__typical_road}) OR (highway=pedestrian AND NOT bicycle=yes AND NOT bicycle = designated))"
...
__weird_highway_value = weird("highway", ["motorway", "motorway_link", "trunk", "trunk_link", "primary", "primary_link", "secondary", "secondary_link", "tertiary", "tertiary_link", "pedestrian", "residential", "living_street", "unclassified", "service", "track", "footway", "cycleway", "steps", "path", "proposed", "construction", "bridleway", "platform", "bus_stop"])
...
if debug
	...
	puts "		only_yes_bicycle_on_cycleway : (highway=cycleway AND bicycle=yes)"
...
puts "		oneway : (((oneway=yes AND NOT #{__contraflow}) OR (oneway:bicycle=yes)) AND (#{__cycleable} OR #{__typical_road}))"
...
puts "		unexpected cycling ban: #{__unexpected_cycling_ban} AND NOT area = yes"
puts "	areas"
...
puts "		OK_bicycle_crossing : way[#{__bicycle_crossing_way}].node[highway=crossing AND (bicycle=yes)]"
puts "		not_OK_bicycle_crossing : way[#{__bicycle_crossing_way}].node[highway=crossing AND bicycle=no]"
if debug
	puts "		not_defined_bicycle_crossing : way[#{__bicycle_crossing_way}].node[highway=crossing AND NOT bicycle=yes AND NOT bicycle=no]"

...
proper_surface = "(surface = asphalt OR surface = metal OR smoothness = excellent OR smoothness = good OR smoothness = intermediate OR cycleway:surface=asphalt)"
terible_surface = "(smoothness = bad OR smoothness = very_bad OR smoothness = horrible OR smoothness = very_horrible OR smoothness = impassable OR surface = mud OR surface=cobblestone OR surface=grass_paver)"
...
valid_bicycle_source_value = "(source:bicycle=sign OR source:bicycle=park_rules OR source:bicycle=forest_rules OR source:bicycle=cemetery_rules OR source:bicycle=water_law OR footway=sidewalk OR footway=crossing OR source:bicycle=PL:not_sidewalk)" #footway=sidewalk, footway=crossing hack is temporary as source:bicycle needs proper string for this status (but it probably will be checked anyway)
OK_surface_values = ["asphalt", "grass", "dirt", "compacted", "sett", "paved", "paving_stones", "gravel", "ground", "sand", "wood", "earth", "pebblestone", "concrete", "concrete:plates", "unpaved", "cobblestone", "mud", "grass_paver", "metal"]
...
if debug
	puts "		public as access value: access=public OR vehicle=public OR bicycle=public"
...
puts "		blocked area: landuse=railway OR aeroway=aerodrome OR man_made=works"
puts "	lines"
